...
const en = {
  ...
  'ng.tooBigBody': 'A dependency graph stays readable up to about a hundred nodes; past that, zooming does not help — narrowing does. Pick a project, or use the scope filter, and every other view narrows with it.',
  ...
  'g.compactTip': 'Pack each group\u2019s bars into as few lanes as they need — one line per set of work that genuinely runs at the same time. A lane holds several tasks, so their names are on the bars rather than in the left column.',
  'g.classicTip': 'One row per task, named in the left column. Honest, and on a plan of this size a staircase of short bars down an empty canvas.',
  'g.loadHeatmap': 'Load', 'g.loadHeatmapTip': 'Two readings of the same fact — how much of that week the person actually had. A solid line under a bar is the weekly load (blue under, green healthy, amber full, red over); a hatched one is time off. Resource group rows carry the week-by-week strip.',
  ...
  'bd.help': 'Restructuring a plan is bookkeeping about work that was already there. Dated today it reads as movement in the next review; dated earlier it does not. What was actually done still counts on the day it was done.',
...
const de = {
  ...
  'ng.tooBigBody': 'Ein Abhängigkeitsnetz bleibt bis etwa hundert Knoten lesbar; darüber hilft Zoomen nicht, nur Eingrenzen. Wähl ein Projekt oder nimm den Ausschnitt-Filter — alle anderen Ansichten grenzen mit ein.',
  ...
  'g.compactTip': 'Die Balken jeder Gruppe in so wenige Spuren packen, wie sie brauchen — eine Zeile je Menge Arbeit, die wirklich gleichzeitig läuft. Eine Spur trägt mehrere Aufgaben, ihre Namen stehen deshalb auf den Balken statt in der linken Spalte.',
  'g.classicTip': 'Eine Zeile je Aufgabe, benannt in der linken Spalte. Ehrlich — und bei dieser Plangröße eine Treppe aus kurzen Balken über eine leere Fläche.',
  'g.loadHeatmap': 'Last', 'g.loadHeatmapTip': 'Zwei Lesarten derselben Sache — wie viel die Person von dieser Woche wirklich hatte. Eine durchgezogene Linie unter einem Balken ist die Wochenlast (blau unterlastet, grün gesund, gelb voll, rot überlastet), eine schraffierte ist Abwesenheit. Ressourcen-Gruppenzeilen tragen den Wochenstreifen.',
  ...
  'bd.help': 'Eine Struktur umzubauen ist Buchhaltung über Arbeit, die vorher schon da war. Auf heute datiert liest sie sich im nächsten Review als Fortschritt, früher datiert nicht. Was wirklich getan wurde, zählt weiterhin an dem Tag, an dem es getan wurde.',
  'bd.date': 'Wirksam zum',
