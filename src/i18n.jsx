...
const en = {
  ...
  'g.reorderFirstTip': 'Scheduler tiebreak: move selection to the front of the team queue. Only takes effect between items with the same priority and due date — capacity and dependencies still dominate.',
  'g.reorderUpTip': 'Scheduler tiebreak: move selection one slot earlier in the team queue. Only resolves ties on equal priority + due date.',
  'g.reorderDownTip': 'Scheduler tiebreak: move selection one slot later in the team queue. Only resolves ties on equal priority + due date.',
  'g.reorderLastTip': 'Scheduler tiebreak: move selection to the end of the team queue. Only takes effect between items with the same priority and due date — for a guaranteed timeline-end position pin the start date or raise the priority number.',
  'g.reorderHintExtreme': 'This is the lowest (or highest) scheduler position your selection can reach while keeping the current priority. It only shifts the visible timeline if there are later items in this team queue at the same priority.',
  'g.reorderHintNudge': 'Selection nudged within the team queue. The visible bars only shift when neighbouring items share the same priority and due date — otherwise priority and capacity win.',
...
const de = {
  ...
  'g.reorderFirstTip': 'Scheduler-Tiebreak: Auswahl an den Anfang der Team-Queue. Wirkt nur zwischen Items mit gleicher Priorität und gleichem Fälligkeitsdatum — Kapazität und Abhängigkeiten dominieren weiterhin.',
  'g.reorderUpTip': 'Scheduler-Tiebreak: Auswahl einen Platz weiter vorne in der Team-Queue. Greift nur bei Gleichstand in Priorität und Fälligkeit.',
  'g.reorderDownTip': 'Scheduler-Tiebreak: Auswahl einen Platz weiter hinten in der Team-Queue. Greift nur bei Gleichstand in Priorität und Fälligkeit.',
  'g.reorderLastTip': 'Scheduler-Tiebreak: Auswahl ans Ende der Team-Queue. Wirkt nur zwischen Items mit gleicher Priorität und Fälligkeit — für garantiertes Timeline-Ende Startdatum pinnen oder Prio-Zahl erhöhen.',
  'g.reorderHintExtreme': 'Das ist die tiefste (bzw. höchste) Position, die deine Auswahl bei dieser Priorität erreichen kann. Sichtbar verschiebt sich der Balken nur, wenn es in dieser Team-Queue noch spätere Items mit gleicher Priorität gibt.',
  'g.reorderHintNudge': 'Auswahl in der Team-Queue verschoben. Die Balken bewegen sich nur, wenn benachbarte Items dieselbe Priorität und Fälligkeit haben — sonst gewinnen Prio und Kapazität.',
