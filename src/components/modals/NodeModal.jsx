          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <label className="toggle" style={{ margin: 0 }}>
              <input type="checkbox" checked={(f.fixedDurationDays || 0) > 0}
                onChange={e => s('fixedDurationDays', e.target.checked ? (f.fixedDurationDays || Math.max(1, Math.ceil(f.best || 1))) : 0)} />
      ...
      {activeNmTab === 'timing' && <>
        ...
        <div className="field">
          ...
          <div ref={focusRefs.deps} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <SearchSelect
              options={allIds.filter(i => !(f.deps || []).includes(i) && !(f.softDeps || []).includes(i)).map(i => ({ id: i, label: findById(i)?.name || '' }))}
              onSelect={id => s('deps', [...new Set([...(f.deps || []), id])])}
              placeholder={`+ ${t('qe.predecessors')}`}
              showIds />
          </div>
        </div>
