...
export function AutoAssignHint({ node, scheduled, members, onAccept }) {
  ...
  return <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 9px', background: 'var(--bg3)', border: '1px solid var(--b2)', borderRadius: 'var(--r)', fontSize: 12, marginBottom: 6 }}>
    <span style={{ color: 'var(--tx3)', flexShrink: 0, fontStyle: 'italic' }}>{t('aa.suggestion')}</span>
    <span style={{ fontWeight: 600 }}>{m.name}</span>
    <span style={{ fontSize: 10, color: 'var(--tx3)', fontFamily: 'var(--mono)' }}>{iso(sc.startD)} — {iso(sc.endD)}</span>
