    ...
    <div className="res-grid">
      {members.map(m => <div key={m.id} className="res-card">
        <div className="res-ch">
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><TBadge t={m.team} teams={teams} />{onClone && <button className="btn btn-sec btn-xs" onClick={() => onClone(m)} title={t('rv.clone')}>{t('rv.clone')}</button>}<button className="btn btn-danger btn-xs" onClick={() => onDel(m.id)}>{t('rv.remove')}</button></div>
        </div>
        {[
