...
describe('tree editor — keyboard model', () => {
  ...
  // This asserted a printed hint line — a wall of glyphs above the table
  // teaching keys for controls you could not see yet. The shortcuts now live
  // on each control's own tooltip, with the full list one keypress away, so
  // what the view owes is the way IN to that list, not the list itself.
  it('offers the keyboard map instead of printing a hint wall', () => {
    const tree = [mk('P1')];
    renderHarness({ initialTree: tree });
    expect(screen.getByTestId('tree-keymap-btn')).toBeTruthy();
    expect(screen.queryByText(/Enter edit/i)).toBeNull();
  });
});
