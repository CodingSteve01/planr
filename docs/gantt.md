...
Both are drawn **inside the bar**, clipped to it, and they say different kinds of thing so they take different shapes.

**Load** is a 3px solid line on the bar's foot, in the heat colours, and follows the Load toggle.

**Absence** is the stretch of the bar where no work happens — the reason five days of work span three weeks — so it is drawn as a *pause in the bar*: full height, diagonally hatched at low opacity with a solid seam at each end. The seams carry the meaning (exactly where the pause starts and stops); the wash between them says it is one stretch rather than two marks; and it is faint enough to read the bar's own label straight through, which the first full-strength version was not — it simply ate the name. It is always drawn, because "somebody was away in the middle of this" is a fact about the work rather than a reading you switch on. It has **no tooltip of its own**: the pointer is already on the bar, whose tooltip is open, and a second box on top of it said the same thing badly. The bar's tooltip lists the vacation instead — dates, person, note.

Three shapes were tried and the first two are worth recording. An amber block at full ROW height on every task row of the person away painted a hundred blocks for one week off. A 3px line stopped the wash but made absence a decoration on the bar: colour saying nothing about what it interrupts, and indistinguishable from the load line beside it. That answers the question they are there for — what the load was while the work was happening, and why five days of work span three weeks — and nothing else competes for the row.
