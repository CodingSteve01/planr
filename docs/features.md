...
- **Single-line roadmap** — the line picker above the map (a searchable dropdown, `Alle Linien` by default, with a colour dot showing the picked line's colour) reduces the map to one project. Eight lines crossing a 1400×800 canvas is a network diagram; one line is a roadmap you can read the station labels on. The soloed line keeps its palette colour (that is its identity across the plan) but is placed on the longest route so its stations spread out. Also scopes the Fahrplan/Timetable. Persisted in `planr_roadmap_solo`; a remembered project that no longer exists silently falls back to all lines. While a line is soloed the renderer is given a colour-only assignment and its `onAssignmentChange` is withheld — otherwise persisting the recomputed route would move that line in the full map.
