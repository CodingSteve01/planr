...
- **Zoom + pan on the map** — both the subway map and the project roadmap are fixed-width SVGs scaled to the container, which on a dense plan means squinting. The `−` / `+` / `Fit` controls in the map's top-right corner (and Ctrl/Cmd + mouse wheel, anchored on the cursor) widen the drawing past its container and let it scroll in both axes; plain wheel still scrolls the page. Page-zoom semantics on purpose — re-laying out at every zoom step would move stations around while the user is trying to read one. Persisted in `planr_roadmap_zoom`; at 100 % the container behaves exactly as before (no scrollbars, no max height).
