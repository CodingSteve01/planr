...
- **The cursor stays on screen** — the tree keeps the selected row inside its scroll container, below the sticky table head, with a row's worth of room on either side. It used to ask whether the row was inside the *window*, but the tree scrolls a container that starts below the topbar, the tab bar and the sub-toolbar: a row that had scrolled up behind that chrome still reported `top >= 0` and counted as visible, so moving the cursor upward it simply vanished and nothing scrolled (measured: container top at y=160, cursor row at y=36). It also used `block: 'nearest'`, which does the least scrolling that technically works and leaves the row glued to the edge with nothing visible after it. The geometry lives in `scrollAdjustment()` in [`utils/treeEdit.js`](../src/utils/treeEdit.js) as a pure function with unit tests — the DOM behaviour itself is layout-dependent and cannot be tested honestly in happy-dom, which has no layout.
- **Moving the cursor does not re-render the whole tree** — arrow-key navigation changes exactly two rows, but it used to re-render every visible one: measured at **82 ms per arrow press on a 300-row plan**, against a 16 ms frame budget. Holding the key queues presses at 82 ms each, which is where "half a second to a second behind" came from. Rows are now cached by signature (`rowCacheRef` in `TreeView.jsx`) and handed back as the same element reference when nothing about them changed, which React skips re-rendering. Same measurement after: **no task over 50 ms at all**. The cost that buys is the opposite failure — a row that quietly stops updating — so the signature names everything the row reads, and per-row state (`editing`, the drag target) is kept per-row so a rename keystroke does not rebuild every row. Pinned by tests that fail loudly if the cache stops invalidating.
  Worth knowing how this was found: the first two measurements were wrong. `requestAnimationFrame` and `setTimeout` are both throttled in a hidden browser tab, which reported a suspiciously round ~1000 ms per press. `PerformanceObserver` on `longtask` is not throttled, and comparing 301 visible rows against 13 (no measurable task) is what ruled out saving, syncing and the side panel and pointed at the row count.
...
- **The conventional file shortcuts work** — `⌘O` open, `⌘S` save, `⇧⌘S` save as, `⌘E` export (Report mode), `⌘,` settings, declared in `utils/shortcuts.js` so the keymap, the File menu and the palette all name exactly what is bound. They stay out of the way while you are typing in a field — except Save, which people expect from anywhere. `⌘N` is deliberately **not** bound: browsers keep it for "new window" and never deliver it to the page, so binding it would look like a feature and do nothing.
- **The header holds still** — the topbar used to `flex-wrap`, and the save status rewrites its own text every few seconds (`saving…` → `all saved · 14:32` → `unsaved · saving in 3s`), so on a narrow window the whole header kept re-sorting itself while you worked and the control you were aiming at moved. It is one row now: the file/save group absorbs the width and truncates, the status pill has a stable width so its own updates nudge nothing, the scheduled/done count drops below 1100px, and everything to the right of it holds its position.
- **File operations have a visible home** — a single **📁 File** button in the topbar opens Load / Save as / Snapshots / Export / New ([`src/components/shared/FileMenu.jsx`](../src/components/shared/FileMenu.jsx)), anchored next to the file-and-save status pill where file state already lives. Phase 3 had moved all five into the `/` palette and left the topbar with `/` and `⚙ Settings`; that was right about the old five-button row being too loud and wrong about these — opening and saving a file is the first thing someone reaches for, not an advanced command they go hunting for ("I'm looking for save/load…"). One button instead of five, and **nothing was taken away**: every entry is still in the palette too, which a test pins. Same popup mechanics as `ViewFilters` (outside-click and Escape close); the Save row shows its `⌘S` from `shortcuts.js` rather than a second hardcoded copy.
- **`/` command palette** (`src/components/shared/CommandPalette.jsx`) — opens with `/` (when focus isn't already in a field) or `⌘K`/`Ctrl+K`; fuzzy-filters a flat command list (`src/utils/palette.js`), `↑↓` + `Enter` to run, `Esc` to close. Every row carries an **icon** and, where one exists, its **shortcut** (read from `shortcuts.js`, so it cannot drift from the keymap) — twenty rows of bare text are a list you read rather than one you scan. Holds everything moved out of the topbar's old button row — Load file, Snapshots, Save as, Export… (jumps to Report mode) / Export… (dialog) (still opens the old modal), New project, Help/Tour — plus a jump to every mode and every view.
...
- **Unified view-filter popup** (⚙ Filter chip in the sub-toolbar) — single trigger that exposes the two global "review/planning" filters:
  ...
  - **Archive** — hide long-finished projects and long-offboarded people (see the Archive filter bullet below), with the day threshold in the popup and what is currently archived on the count’s tooltip.
...
- **The editor goes where there is room** — the item editor sits beside the tree while the app is wider than 1180px and switches to a dialog when it is not, measured on the app itself rather than the window (inside Obsidian a pane is narrow far more often than a window is). Settings → *Item editor* pins it to `Auto`, `Side panel` or `Dialog`. The panel header's `⇥` sends the editor to a dialog and hands the open item over with it; the dialog's `⇤` brings it back. Either way the row itself carries a `⊞` that opens the editor — the standard move for a table row, and the one path that does not depend on a panel being there. State in `planr_editor_dock`.
- **IDs are optional** — Settings → *Item IDs in the tree* hides the ID column. Five dotted segments in front of every name is the tool's spine when you are wiring dependencies and noise when you are reading. Hidden, the id still stands in the selection bar, the editor, the tooltip and every export; the drag handle keeps its column. State in `planr_tree_ids`.
