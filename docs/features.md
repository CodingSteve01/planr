...
- **The editor goes where there is room** — the item editor sits beside the tree while the app is wider than 1180px and switches to a dialog when it is not, measured on the app itself rather than the window (inside Obsidian a pane is narrow far more often than a window is). Settings → *Item editor* pins it to `Auto`, `Side panel` or `Dialog`; the panel header has a one-click "send it to a dialog". With the editor docked as a dialog, the selection bar above the tree carries the way in. State in `planr_editor_dock`.
