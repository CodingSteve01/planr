...
- **Soll/Ist comparison** — every done leaf gets a workday-bereinigt estimate-vs-actual breakdown in QuickEdit (Soll = `best × factor`, Ist = workdays between `completedStart` and `completedEnd` with weekends + holidays subtracted, Δ shown as percent with green/amber/red zones at ±20 %). When the Δ Review filter is on, an additional **Retro · Soll/Ist** band aggregates over every leaf completed in the window: totals + ratio + hit rate + Top overruns/underruns lists for one-click drill-in. Lives in [`src/utils/sollIst.js`](../src/utils/sollIst.js) — `computeSollIst(node, ctx)` for the per-task case, `aggregateSollIst(nodes, ctx)` for the retro panel.
