...
- **Unified view-filter popup** (⚙ Filter chip in the sub-toolbar) — single trigger that exposes the two global "review/planning" filters:
  ...
  Filter state lives at the App level (`planr_diff_since`, `planr_diff_only_changed`, `planr_horizon`, `planr_horizon_only`) and is shared across Roadmap, Tree, Gantt, Network and Timetable.
...
- **One progress formula everywhere** — the headline percentage, the delivered-PT pill and their formatting come from [`src/utils/progress.js`](../src/utils/progress.js) (`effortWeightedProgress`, `deliveredEffort`, `totalEffort`, `progressPctLabel`). Overview, the HTML report, the Word report and all four PDFs read the same helpers via `buildReportModel()`, so an export can never print a different number than the screen. Progress is effort-weighted and phase-aware — *not* `done / total` leaf counting.
