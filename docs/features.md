...
- **Single-project roadmap** — picking one project in the line picker (a searchable dropdown above the map, `All lines` by default) switches to a different renderer: [`src/utils/projectRoadmap.js`](../src/utils/projectRoadmap.js) draws a **calendar**, not a metro line. One row per work package, positioned and sized by its real dates, its leaves as stops on that row, a month (or quarter) axis, today as a green line through every row and the root's deadline as a red one. Rows carry the shared `progressPctLabel` figure and `done/total`, and every row and stop keeps the same `data-tip` / `data-item-id` contract as the subway map, so hover and click-to-open work unchanged. A subway line for a single project answers no question you would actually ask about a project; a calendar does. Also goes into the **Management Summary PDF**: one calendar page per project (checkbox `+ project roadmaps`, on by default), rendered for *every* root regardless of what the screen is filtering. Persisted in `planr_roadmap_solo`; a remembered project that no longer exists falls back to all lines. Also scopes the Timetable. While one project is picked the renderer gets a colour-only line assignment and its `onAssignmentChange` is withheld — otherwise persisting the recomputed route would move that line in the full map.
